...
//  DONE (18) Create a String array containing the names of the desired data columns from our ContentProvider
    private static final String[] WEATHER_DATA_COLUMNS = {
...
//  DONE (19) Create constant int values representing each column name's position above
    ...
    public static final int INDEX_DEGREES = 1;
...
//  DONE (20) Create a constant int to identify our loader used in DetailActivity
    private final int INDEX_LOADER = 322;
...
//      DONE (35) Initialize the loader for DetailActivity
        ...
        loaderManager.initLoader(INDEX_LOADER, null, this);
...
//  DONE (24) Override onLoadFinished
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        if(data != null && data.moveToFirst()){
            ...
            String windString =
                    SunshineWeatherUtils.getFormattedWind(this,
                            data.getFloat(INDEX_WIND),
                            data.getFloat(INDEX_DEGREES));
            String highString = "High: " +
                    SunshineWeatherUtils.formatTemperature(this, data.getFloat(INDEX_MAX))
                    + "C";

            String lowString = "Low: " +
                    SunshineWeatherUtils.formatTemperature(this, data.getFloat(INDEX_MIN))
                    + "C";
            String dateString = SunshineDateUtils.getFriendlyDateString(this,
                    data.getLong(INDEX_DATE), true);

            mDate.setText(dateString);
            mDescription.setText(descriptionString);
            mHigh.setText(highString);
            mLow.setText(lowString);
            mHumidity.setText(data.getFloat(INDEX_HUMIDITY) + "%");
            mPressure.setText(data.getString(INDEX_PRESSURE) + " mph");
            mWind.setText(windString);

            String summary = descriptionString + highString + lowString;
